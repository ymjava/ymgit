...
public class LiantongSmsUtil {
    ...
    private static LiantongSmsUtil instance = null;   
    private static Object lock = new Object(); 
	
    public static LiantongSmsUtil getInstance() {   
        if(instance==null){  
            synchronized (lock) {  
                if(instance==null)  
                    instance = new LiantongSmsUtil();  
           }  
        }  
        return instance;  
    }
    private LiantongSmsUtil(){  
        try {  
           init();  
       } catch (Exception e) {  
    ...
    { 
        ...
        gateway = new SerialModemGateway("modem."+PropertieUtil.loadPropertie("sms_liantong_com_index"),
				PropertieUtil.loadPropertie("sms_liantong_com"), Integer.parseInt(PropertieUtil.loadPropertie("sms_liantong_pote")), "wavecom", "17254");
       //不接受短信
        gateway.setSimPin(PropertieUtil.loadPropertie("liantong_pin"));
